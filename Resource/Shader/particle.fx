...
//단순 배열로 생각해도 좋다. 1000개 짜리의
RWStructuredBuffer<Particle> g_particle : register(u0);
//크기가 1인 배열임. 공용 데이터를 넣는 용도
RWStructuredBuffer<ComputeShared> g_shared : register(u1);

/* CS_Main
* g_vec2_1 : DeltaTime(이전~현재 시간)  / AccTime(누적된 시간) Random Seed value
* g_int_0 : Particle Max Count
* g_int_1 : AddCount
* g_vec4_0 : MinLifeTime / MaxLifeTime / MinSpeed / MaxSpeed
* Material에서 Dispatch함
* Dispatch Group은 (1, 1, 1)로 setting
*/
[numthreads(1024, 1, 1)]
void CS_Main(int3 threadIndex : SV_DispatchThreadID)
{
	...
	//현재 쓰레드 인덱스를 참고해서 이전에 있던 애 값에 뭔가 해주겠다고 연산한다면,
	// 큰 문제를 야기한다. -> 순서가 보장이 되지 않는다.
	//g_particle[threadIndex.x] = g_particle[threadIndex.x -1];
	
	//g_shared같은 경우 1024개가 동시에 쓰는 공용 자원.
	...
	if (g_particle[threadIndex.x].alive == 0) //만약 죽어있는 값이라면,
	{
		while (true)
		{
			...
			//expected가 addcount와 같다면 desired를 addCount에 넣어주겠다.
			//최종적으로 addCount는 originalValue에 넣는다.
			/*
				의사 코드
